...
def update_show_status(show_status, item):
    ...
    if item.guid not in show_status["recentGuids"]:
        show_status["recentGuids"] = ([item.guid] + show_status["recentGuids"])[
            : CONFIG["recentItemsCount"]
        ]

    if item.tv_info_hash not in show_status["recentHashes"]:
        show_status["recentHashes"] = (
            [item.tv_info_hash] + show_status["recentHashes"]
        )[: CONFIG["recentItemsCount"]]
    return show_status


for item in showrss.entries:
    show_status = status["shows"].get(
        item.tv_show_id,
        {
            "name": "",
            "lastPubDate": datetime.min.replace(tzinfo=pytz.UTC),
            "recentGuids": [],
            "recentHashes": [],
        },
    )
